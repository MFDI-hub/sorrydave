...
def build_match_public_key_payload(
    user_id: int,
    public_key: bytes,
    key_version: int,
) -> dict[str, object]:
    """
    Build the JSON body for POST ``/voice/<user-id>/match-public_key``.

    No I/O; the caller sends the request. Protocol.md persistent verification:
    confirm that another user's presented key matches the uploaded persistent key.

    Args:
        user_id (int): Verified user's Discord snowflake.
        public_key (bytes): P-256 public key (X9.62) presented in the call.
        key_version (int): Persistent key version (e.g. 1).

    Returns:
        dict[str, object]: ``user_id``, ``public_key``, ``key_version``.
    """
    return {
        "user_id": int(user_id),
        "public_key": public_key,
