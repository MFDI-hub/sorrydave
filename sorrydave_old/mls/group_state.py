...
Public functions:
    ...
    iter_member_keys: Occupied leaves as (leaf_index, identity, signature_key).
"""
...
def _p256_public_from_der(private_key_der: bytes) -> bytes:
    """Return uncompressed X9.62 P-256 public key from a PKCS#8 DER private key."""
    ...
    if not isinstance(key, ec.EllipticCurvePrivateKey) or not isinstance(
        ...
        raise ValueError("signing_key_der must be a P-256 EC private key")
    sig_public_bytes = key.public_key().public_bytes(
        serialization.Encoding.X962,
        serialization.PublicFormat.UncompressedPoint,
    )
    if sig_public_bytes[0] != 0x04:
        sig_public_bytes = b"\x04" + sig_public_bytes
    return sig_public_bytes
...
def _p256_signing_keypair() -> tuple[bytes, bytes]:
    ...
    return sig_private_der, _p256_public_from_der(sig_private_der)
...
def create_key_package(
    ...
    *,
    signing_key_der: Union[bytes, None] = None,
    ...
    Args:
        ...
        signing_key_der (Union[bytes, None]): Optional PKCS#8 DER P-256 signature private
            key (persistent identity). When omitted, a new ephemeral keypair is generated.
    ...
    Raises:
        ValueError: If MLS ciphersuite is unknown or signing_key_der is not P-256.
    """
    if crypto is None:
    ...
    if signing_key_der is None:
        ...
        sig_private_der = signing_key_der
        sig_public_bytes = _p256_public_from_der(signing_key_der)
...
def iter_member_keys(session: MLSGroupSession) -> list[tuple[int, bytes, bytes]]:
    """
    Return (leaf_index, identity, signature_key) for occupied leaves.

    Identity is the basic-credential identity (big-endian user ID snowflake).
    ``signature_key`` is the leaf P-256 public key used for identity verification.
    Blank tree slots are omitted.
    """
    out: list[tuple[int, bytes, bytes]] = []
    try:
        tree = session._group._inner._ratchet_tree
        for leaf_index in range(tree.n_leaves):
            ...
            if leaf.credential is not None:
                identity = getattr(leaf.credential, "identity", b"") or b""
            signature_key = getattr(leaf, "signature_key", b"") or b""
            out.append((leaf_index, identity, signature_key))
    except Exception:
        return []
    return out
